...
public class PhoneNumberPricing {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if(phoneNumber.equals("-e"))
                return;
            System.out.println("Price: " + calculatePrice(phoneNumber) + " sum");
        }
    }

    public static double calculatePrice(String phoneNumber) {
        if (phoneNumber.length() != 7 || !phoneNumber.matches("[0-9]+"))
            throw new IllegalArgumentException("Invalid phoneNumber");

        if (phoneNumber.matches("0000000") || phoneNumber.matches("7777777"))
            return 52625000;
            ...
            return 1500000;
        else if(phoneNumber.matches("777\\d{4}"))
            return 1000000;
